...
namespace Aladdin.CAPI.ANSI.Hash
{
    ...
    ///////////////////////////////////////////////////////////////////////////////
    public class RIPEMD256 : BlockHash
    {
        ...
        private static uint F1(uint a, uint b, uint c, uint d, uint x, int s)
        {
            return RL(a + f1(b, c, d) + x, s);
        }
        private static uint F2(uint a, uint b, uint c, uint d, uint x, int s)
        {
            return RL(a + f2(b, c, d) + x + 0x5a827999, s);
        }
        private static uint F3(uint a, uint b, uint c, uint d, uint x, int s)
        {
            return RL(a + f3(b, c, d) + x + 0x6ed9eba1, s);
        }
        private static uint F4(uint a, uint b, uint c, uint d, uint x, int s)
        {
            return RL(a + f4(b, c, d) + x + 0x8f1bbcdc, s);
        }
        private static uint FF1(uint a, uint b, uint c, uint d, uint x, int s)
        {
            return RL(a + f1(b, c, d) + x, s);
        }
        private static uint FF2(uint a, uint b, uint c, uint d, uint x, int s)
        {
          return RL(a + f2(b, c, d) + x + 0x6d703ef3, s);
        }
        private static uint FF3(uint a, uint b, uint c, uint d, uint x, int s)
        {
          return RL(a + f3(b, c, d) + x + 0x5c4dd124, s);
        }
        private static uint FF4(uint a, uint b, uint c, uint d, uint x, int s)
        {
          return RL(a + f4(b, c, d) + x + 0x50a28be6, s);
        }
        private long byteCount; private uint H0, H1, H2, H3, H4, H5, H6, H7;
	    ...
	    protected override void Update(byte[] data, int dataOff)  
        {
            ...
            uint a  = H0; uint b  = H1; uint c  = H2; uint d  = H3;
            uint aa = H4; uint bb = H5; uint cc = H6; uint dd = H7; uint t; 
            //
            // Round 1
            //
            a = F1(a, b, c, d, X[ 0], 11);
            ...
            byte[] data, int dataOff, int dataLen, byte[] buf, int bufOff)
        {
            ...
            Math.Convert.FromUInt32(H0, Endian, buf, bufOff +  0); 
            Math.Convert.FromUInt32(H1, Endian, buf, bufOff +  4); 
            Math.Convert.FromUInt32(H2, Endian, buf, bufOff +  8); 
            Math.Convert.FromUInt32(H3, Endian, buf, bufOff + 12); 
            Math.Convert.FromUInt32(H4, Endian, buf, bufOff + 16); 
            Math.Convert.FromUInt32(H5, Endian, buf, bufOff + 20); 
            Math.Convert.FromUInt32(H6, Endian, buf, bufOff + 24); 
            Math.Convert.FromUInt32(H7, Endian, buf, bufOff + 28); 
        }
        ////////////////////////////////////////////////////////////////////////////
