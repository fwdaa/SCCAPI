...
namespace Aladdin.CAPI.ANSI.Hash
{
    ...
    public class MD2 : BlockHash
    {
        ...
        private byte[] X = new byte[48]; // X buffer
        private byte[] C = new byte[16]; // check sum
            ...
            byte[] data, int dataOff, int dataLen, byte[] buf, int bufOff)
        {
            ...
            Array.Copy(X, 0, buf, bufOff, 16);
        }
        private void ProcessBlock(byte[] data, int dataOff)
        {
            for (int i = 0; i < 16; i++)
            {
                X[i + 16] = (byte)(data[dataOff + i]       ); 
