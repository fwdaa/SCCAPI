...
///////////////////////////////////////////////////////////////////////////////
public class SHA3 extends BlockHash
{
    ...
    private static final long[] KeccakRoundConstants = new long[24];
    private static final int [] KeccakRhoOffsets     = new int [25];
    static {
    ...
    private static void chi(long[] A)
    {
        ...
        for (int y = 0; y < 5; y++)
        {
            ...
            System.arraycopy(chiC, 0, A, 5 * y, 5);
        }
    }
    ...
    private static void keccakPermutation(byte[] state)
    {
        ...
        {
            longState[i] = Convert.toInt64(state, i * 8, ENDIAN); 
        }
        for (int i = 0; i < 24; i++)
        {
        ...
        for (int i = 0; i < longState.length; i++)
        {
           Convert.fromInt64(longState[i], ENDIAN, state, i * 8);
        }
    }
    // внутренее состояние
    private final byte[] state = new byte[200]; 
    
    // размер хэш-значения и блока
    private final int hashSize; private final int blockSize;
	...
	@Override public int hashSize() { return hashSize; }  
    ...
    { 
        // инициализировать алгоритм
        super.init(); Arrays.fill(state, (byte)0);
    }
        ...
        byte[] data, int dataOff, int dataLen, byte[] buf, int bufOff)
    {
        ...
        System.arraycopy(data, dataOff, buffer, 0, dataLen); update(buffer, 0); 

        // извлечь байты из состояния
        System.arraycopy(state, 0, buffer, 0, blockSize);
        ...
        for (int offset = 0; offset < hashSize; offset += cb, bytesAvailable -= cb)
        {
            ...
            System.arraycopy(buffer, blockSize - bytesAvailable, buf, bufOff + offset, cb);
        }
    }
    ////////////////////////////////////////////////////////////////////////////
    // Тесты известного ответа
    ////////////////////////////////////////////////////////////////////////////
    public static void test224(Hash hashAlgorithm) throws Exception
    {
    ...
    public static void test256(Hash hashAlgorithm) throws Exception
    {
        knownTest(hashAlgorithm, 1, 
            "", new byte[] {
    ...
    public static void test384(Hash hashAlgorithm) throws Exception
    {
        knownTest(hashAlgorithm, 1, 
            "", new byte[] {
    ...
    public static void test512(Hash hashAlgorithm) throws Exception
    {
        knownTest(hashAlgorithm, 1, 
            "", new byte[] {
