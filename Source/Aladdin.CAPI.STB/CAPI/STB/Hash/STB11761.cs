...
{
    public class STB11761 : BlockHash
    {
	    ...
	    //////////////////////////////////////////////////////////////////////////////
	    private static readonly uint[] StartV =
	    {
	    ...
	    private static readonly uint[] T5 =
	    {
	    ...
	    private static readonly uint[] T7 =
	    {
	    ...
	    //////////////////////////////////////////////////////////////////////////////
	    private static void rho(uint[] V, uint[] T)
	    {
            ...
            Array.Copy(V, 29, T,  0, 16);
		    Array.Copy(V, 47, T, 16, 16);
		    Array.Copy(V, 66, T, 32, 16);
		    Array.Copy(V, 83, T, 48, 16);
		    Array.Copy(T, 48, V,  0, 16);
	    }
	    private static void omega(uint[] X, uint xOff, uint[] Y0, uint[] Y1, uint[] Y2,
		     uint[] Y3, uint[] Y4, uint[] Y5, uint[] Y6, uint[] Y7)
	    {
		    ...
		    {
                Math.Convert.FromUInt32(Y0[i], Endian, y0, i * 4); 
                Math.Convert.FromUInt32(Y1[i], Endian, y1, i * 4); 
                Math.Convert.FromUInt32(Y2[i], Endian, y2, i * 4); 
                Math.Convert.FromUInt32(Y3[i], Endian, y3, i * 4); 
                Math.Convert.FromUInt32(Y4[i], Endian, y4, i * 4); 
                Math.Convert.FromUInt32(Y5[i], Endian, y5, i * 4); 
                Math.Convert.FromUInt32(Y6[i], Endian, y6, i * 4); 
                Math.Convert.FromUInt32(Y7[i], Endian, y7, i * 4); 
		    }
		    for (int m = 0; m < 32; m++)
		    {
			    // вычисляем вектор подстановок
			    uint P0  = (uint)(y0[(X[xOff + 0] >>  0) & 0xFF] <<  0);
				    P0  |= (uint)(y1[(X[xOff + 0] >>  8) & 0xFF] <<  8);
				    P0  |= (uint)(y2[(X[xOff + 0] >> 16) & 0xFF] << 16);
				    P0  |= (uint)(y3[(X[xOff + 0] >> 24) & 0xFF] << 24);
			    uint P1  = (uint)(y4[(X[xOff + 1] >>  0) & 0xFF] <<  0);
				    P1  |= (uint)(y5[(X[xOff + 1] >>  8) & 0xFF] <<  8);
				    P1  |= (uint)(y6[(X[xOff + 1] >> 16) & 0xFF] << 16);
				    P1  |= (uint)(y7[(X[xOff + 1] >> 24) & 0xFF] << 24);

			    // циклический сдвиг на 3 разряда влево
			    uint Carry = P1 >> 29; P1 = (P1 << 3) | (P0 >> 29); P0 = (P0 << 3) | Carry;
	    ...
	    private static void powerXi(uint[] WI, uint[] WO)
	    {
		    WO[7] = WI[0] ^ WI[2];
		    WO[2] = WI[0] ^ WI[4] ^ WI[6];
		    WO[3] = WI[1] ^ WI[5] ^ WI[7];
	    ...
	    private static void doublePhi(uint[] H, int hOff, uint[] W)
	    {
		    uint Temp = 0;
		    for (int j = 0; j < 4; j++)
		    {
	    ...
	    private uint[] hash;    // текущее хэш-значение
	    private uint[] vec;   	// вспомогательный вектор
	    private ulong  length;	// размер данных

        public STB11761(byte[] start) : base()
	    { 
		    // выделить память для текущего хэш-значения
		    this.start = start; hash = new uint[8]; vec = new uint[99]; 
	    }
	    ...
	    //////////////////////////////////////////////////////////////////////////////
	    private void ProcessBlock(uint[] src, int srcOff)
	    {
		    // временные переменные
		    uint[] T0 = new uint[64]; uint[] T2 = new uint[64];
		    uint[] T4 = new uint[64]; uint[] T6 = new uint[64];
		    uint[] WI = new uint[ 8]; uint[] WO = new uint[ 8];
	    ...
	    public override void Init()
	    {
		    ...
		    base.Init(); length = 0; Array.Copy(StartV, 0, vec, 0, StartV.Length); 
        
		    // для всех четверок байтов
		    for (int i = 0; i < start.Length / 4; i++)
		    {
			    // преобразовать в число
			    hash[i] = Math.Convert.ToUInt32(start, i * 4, Endian); 
		    }
	    }
	    protected override void Update(byte[] src, int srcOff)
	    {
		    // создать временные переменные
		    uint[] srcI = new uint[BlockSize / 4];  

		    // для всех четверок байтов
		    for (int i = 0; i < srcI.Length; i++)
		    {
			    // преобразовать формат исходных данных
			    srcI[i] = Math.Convert.ToUInt32(src, srcOff + i * 4, Endian); 
		    }
	    ...
	    protected override void Finish(byte[] data, int dataOff, int dataLen, byte[] buf, int bufOff)
	    {
		    ...
		    byte[] buffer = new byte[BlockSize]; Array.Copy(data, dataOff, buffer, 0, dataLen);

		    // обработать неполный блок
		    Update(buffer, 0); length -= (ulong)(buffer.Length - dataLen);

		    // закодировать размер в байтах
            Math.Convert.FromUInt64(length, Endian, buffer, 0); 
        
		    // дополнить блок нулями
		    for (int i = 8; i < buffer.Length; i++) buffer[i] = 0;

		    // обработать размер всех данных в байтах
		    Update(buffer, 0); length -= (ulong)buffer.Length;
        
		    // для всех четверок байтов
		    for (int i = 0; i < buffer.Length / 4; i++)
		    {
			    // извлечь отдельные байты
                Math.Convert.FromUInt32(hash[i], Endian, buf, bufOff + i * 4); 
		    }
	    }
	    ///////////////////////////////////////////////////////////////////////////
