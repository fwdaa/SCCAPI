...
Aladdin::CAPI::CSP::Microsoft::RSA::BaseProvider::CreateAlgorithm(
...
{$
	for (int i = 0; i < 1; i++)
	{
		...
		if (type == CAPI::Hash::typeid)
		{
			...
			if (oid == ASN1::ANSI::OID::rsa_md2	 ) return gcnew Hash::MD2 (this, Handle);
			if (oid == ASN1::ANSI::OID::rsa_md4	 ) return gcnew Hash::MD4 (this, Handle);
