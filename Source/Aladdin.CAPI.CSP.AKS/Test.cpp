	...
	public ref class Test abstract sealed
	{
		public: static void Entry()
		{
			...
			SecurityInfo^ info = gcnew SecurityInfo(
				...
				Scope::System, "Card\\AKS ifdh 0\\CAPI-TEST"
			); 
			// указать провайдер
			Using<Provider^> providerAKS(gcnew Provider()); 
			{ 
				Provider^ provider = providerAKS.Get(); 
				...
				array<int>^ rsaBits = gcnew array<int> { 512, 1024 }; 
