...
///////////////////////////////////////////////////////////////////////////
public class ECKeyAgreement2012 extends ECKeyAgreement
{
    // конструктор
    public ECKeyAgreement2012(KeyDerive keyDerive) { super(keyDerive); } 
    // конструктор
    public ECKeyAgreement2012() { super(); } 
    
    // создать алгоритм хэширования
    @Override protected final Hash createHashAlgorithm(
        IPrivateKey privateKey, int keySize) 
    { 
        // вернуть алгоритм хэширования
        if (keySize == 32) return new aladdin.capi.gost.hash.GOSTR3411_2012(256); 
        
        // вернуть алгоритм хэширования
        if (keySize == 64) return new aladdin.capi.gost.hash.GOSTR3411_2012(512);
    ...
    protected static void knownTest(Factory factory, SecurityObject scope, 
        ...
        BigInteger qx1, BigInteger qy1, BigInteger d1, 
        BigInteger qx2, BigInteger qy2, BigInteger d2, 
        byte[] random, byte[] check) throws Exception
    {
        // указать идентификатор алгоритма хэширования
        String hashOID = (check.length == 32) ? OID.GOSTR3411_2012_256 : OID.GOSTR3411_2012_512;
        
        // указать фабрику кодирования
        ECKeyFactory keyFactory = new ECKeyFactory(keyOID); 
        
        // создать параметры алгоритма
        IECParameters keyParameters = new ECNamedParameters2012(paramOID, hashOID); 
        
        // создать открытый ключ
        IPublicKey publicKey1 = new ECPublicKey(
            keyFactory, keyParameters, new ECPoint(qx1, qy1)
        ); 
        // создать личный ключ
        try (IPrivateKey privateKey1 = new ECPrivateKey(
            factory, null, keyOID, keyParameters, d1))
        {
            // создать открытый ключ
            IPublicKey publicKey2 = new ECPublicKey(
                keyFactory, keyParameters, new ECPoint(qx2, qy2)
            ); 
            // создать личный ключ
            try (IPrivateKey privateKey2 = new ECPrivateKey(
    ...
    public static void test(Factory factory, SecurityObject scope, 
        IKeyAgreement keyAgreement, int[] keySizes) throws Exception
    {
        if (KeySizes.contains(keySizes, 32)) 
        knownTest(factory, scope, keyAgreement, 
            OID.GOSTR3410_2012_512, OID.ECC_TC26_2012_512A, 
