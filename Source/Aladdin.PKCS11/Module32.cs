...
namespace Aladdin.PKCS11
{
	...
	public sealed class Module32 : Module
	{
		...
		private API32.CK_FUNCTION_LIST funcList; private Info info; private bool initialized; 

		// конструктор
        [SecuritySafeCritical]
        [SecurityPermission(SecurityAction.Demand, UnmanagedCode = true)]
		public Module32(API.CK_GETFUNCTIONLIST getFunctionlist) 
        {
            // получить адрес списка функций
            IntPtr ptrFunctionList; Exception.Check(getFunctionlist(out ptrFunctionList)); 

            // выполнить преобразование типа
            funcList = (API32.CK_FUNCTION_LIST)Marshal.PtrToStructure(
                ptrFunctionList, typeof(API32.CK_FUNCTION_LIST)
            ); 
            // создать параметры инициализации
            API32.CK_C_INITIALIZE_ARGS initializeArgs = new API32.CK_C_INITIALIZE_ARGS(); 
            ...
            try {  
                ...
                CK_RV code = funcList.C_Initialize(ptr); API32.CK_INFO ckInfo;

                // проверить инициализацию библиотеки
                if (code == API.CKR_CRYPTOKI_ALREADY_INITIALIZED) initialized = false; 

                // проверить отсутствие ошибок
                else { Exception.Check(code); initialized = true; }
                try { 
        ...
        {
            // освободить выделенные ресурсы
            try { if (initialized) funcList.C_Finalize(IntPtr.Zero); }

            // игнорировать возможное исключение (JaCarta)
            catch (AccessViolationException) {} base.OnDispose(); 
        } 
        // размер целого числа
        public override int LongSize { get { return 4; }} 
            ...
            ulong hSession, ulong hObject, Attribute[] attributes)
        {
            ...
            try { 
                // выделить буфер требуемого размера
                API32.CK_ATTRIBUTE[] attrs = new API32.CK_ATTRIBUTE[attributes.Length]; 
		...
		public override ulong GenerateKey(ulong hSession, Mechanism parameters, Attribute[] attributes)
        {
            ...
            { 
	            ...
	            API32.CK_MECHANISM mechanism = new API32.CK_MECHANISM(
