...
Aladdin::CAPI::CSP::Tumar::SecretKeyType::ConstructKey(
	CAPI::CSP::ContextHandle^ hContext, array<BYTE>^ value, DWORD flags)
{$
	// вызвать базовую функцию
	Using<CAPI::CSP::KeyHandle^> hKey(Microsoft::SecretKeyType::ConstructKey(hContext, value, flags)); 

	// установить нулевую синхропосылку
	BYTE iv[16] = {0}; hKey.Get()->SetParam(KP_IV, IntPtr(iv), 0); return hKey.Detach();
}

array<BYTE>^ Aladdin::CAPI::CSP::Tumar::SecretKeyType::GetKeyValue(
