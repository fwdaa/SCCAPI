namespace Aladdin.CAPI.GOST.Engine
{
    ...
    public class GOSTR3412_M : GOST28147
    {
        ...
        public static void Test(IBlockCipher blockCipher) 
        {
            ...
            using (CAPI.Cipher cipher = blockCipher.CreateBlockMode(modeECB))
            { 
                // выполнить тест
                CAPI.Cipher.KnownTest(cipher, PaddingMode.None, key, new byte[] {
                ...
                CAPI.Cipher.KnownTest(cipher, PaddingMode.None, key, data, new byte[] {
            ...
            using (CAPI.Cipher cipher = blockCipher.CreateBlockMode(modeECB))
            {
                ...
                using (KeyDerive keyMeshing = new Derive.ACPKM(cipher))
                {
                    ...
                    using (CAPI.Cipher cipherCTR = new Mode.GOSTR3412.CTR(
                        cipher, ctrParameters, keyMeshing, 16))
                    {
                        key = new byte[] {
                        ...
                        // выполнить тест
                        CAPI.Cipher.KnownTest(cipherCTR, PaddingMode.None, key, data, new byte[] {
        ...
        // GOSTR3412-MAC 
        ////////////////////////////////////////////////////////////////////////////
        public static void TestMAC(IBlockCipher blockCipher) 
        {
            // указать начальную синхропосылку
            byte[] start = new byte[blockCipher.BlockSize]; 
        
            // создать алгоритм выработки имитовставки
            using (Mac macAlgorithm = CAPI.MAC.OMAC1.Create(blockCipher, start))
            {
            ...
            {
                // создать алгоритм выработки имитовставки
                using (Mac macAlgorithm = MAC.GOSTR3412ACPKM.Create(cipher, 16, 80, 8))
                {
                    byte[] key = new byte[] {
                         (byte)0x88, (byte)0x99, (byte)0xAA, (byte)0xBB, 
                         (byte)0xCC, (byte)0xDD, (byte)0xEE, (byte)0xFF,
