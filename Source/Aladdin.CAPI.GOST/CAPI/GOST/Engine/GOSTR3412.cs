...
namespace Aladdin.CAPI.GOST.Engine
{
    ...
    public class GOSTR3412 : CAPI.Cipher
    {
	    ...
	    public class Encryption : BlockTransform
	    {
		    ...
		    {
                // скопировать данные
                byte[] result = new byte[16]; Array.Copy(src, srcOff, result, 0, 16); 
                ...
                Array.Copy(result, 0, dest, destOff, 16); 
            }
	    }
	    ///////////////////////////////////////////////////////////////////////////
        ...
        private static byte Linear(byte[] a)
        {
            ...
            byte[] kB = { 148, 32, 133, 16, 194, 192, 1, 251, 1, 192, 194, 16, 133, 32, 148, 1 }; 

            // выполнить линейное преобразование
            byte sum = 0; for (int i = 0; i < 16; ++i) sum ^= MultTable[a[i] * 256 + kB[i]]; return sum; 
        }
        private static void R(byte[] a, byte[] result)
        {
            // выполнить линейное преобразование и сдвиг
            byte sum = Linear(a); Array.Copy(a, 0, result, 1, 15); result[0] = sum; 
        }
        private static void ReverseR(byte[] a, byte[] result)
        {
            // выполнить циклический сдвиг
            byte a0 = a[0]; Array.Copy(a, 1, result, 0, 15); 
        ...
        private static byte[][] ExpandKey(byte[] key)
        {
            ...
            Array.Copy(key,  0, keys[0], 0, 16); 
            Array.Copy(key, 16, keys[1], 0, 16); 

            // для оставшихся ключей
            for (int i = 0; i < 4; i++)
            {
                // установить начальное значение ключей
                Array.Copy(keys[2 * i    ], 0, keys[2 * i + 2], 0, 16); 
                Array.Copy(keys[2 * i + 1], 0, keys[2 * i + 3], 0, 16); 
                ...
                for (int j = 1; j <= 8; j++)
                {
                    ...
                    Array.Copy(keys[2 * i + 2], 0, keys[2 * i + 3], 0, 16); 
                    Array.Copy(C              , 0, keys[2 * i + 2], 0, 16); 
                }
            }
            return keys; 
        }
        ...
        ///////////////////////////////////////////////////////////////////////
        private static readonly byte[] SBoxZ = {
        ...
        ///////////////////////////////////////////////////////////////////////
        private static readonly byte[] ReversePi = {
        ...
        ///////////////////////////////////////////////////////////////////////
        private static readonly byte[] MultTable = {
        ...
        ////////////////////////////////////////////////////////////////////////////
        public static void Test64(IBlockCipher blockCipher) 
        {
            byte[] key = new byte[] {
        ...
        ////////////////////////////////////////////////////////////////////////////
        public static void Test128(IBlockCipher blockCipher) 
        {
            byte[] key = new byte[] {
                (byte)0x88, (byte)0x99, (byte)0xaa, (byte)0xbb, 
                (byte)0xcc, (byte)0xdd, (byte)0xee, (byte)0xff, 
            ...
            using (CAPI.Cipher cipher = blockCipher.CreateBlockMode(modeECB))
            {
                ...
                using (KeyDerive keyMeshing = new Derive.ACPKM(cipher))
                {
                    ...
                    // создать режим CBC со специальной сменой ключа
                    using (CAPI.Cipher cipherCTR = new Mode.GOSTR3412.CTR(
                        cipher, ctrParameters, keyMeshing, 32))
                    {
                        key = new byte[] {
                            (byte)0x88, (byte)0x99, (byte)0xAA, (byte)0xBB, 
                            (byte)0xCC, (byte)0xDD, (byte)0xEE, (byte)0xFF,
        ...
        // GOSTR3412-MAC 
        ////////////////////////////////////////////////////////////////////////////
        public static void TestMAC64(IBlockCipher blockCipher) 
        {
        ...
        public static void TestMAC128(IBlockCipher blockCipher)
        {
            // указать начальную синхропосылку
            byte[] start = new byte[blockCipher.BlockSize]; 
            ...
            {
                // создать алгоритм выработки имитовставки
                using (Mac macAlgorithm = MAC.GOSTR3412ACPKM.Create(cipher, 32, 96, 16))
                {
                    byte[] key = new byte[] {
                         (byte)0x88, (byte)0x99, (byte)0xAA, (byte)0xBB, 
                         (byte)0xCC, (byte)0xDD, (byte)0xEE, (byte)0xFF,
