...
///////////////////////////////////////////////////////////////////////////////
// Передача строкового представления класса с функцией const char* name()
///////////////////////////////////////////////////////////////////////////////
#define WPP_LOG_NAME(x)     WPP_LOGASTR((x).name())
...
inline size_t strcspn(const char* string, const char* control)
{
	...
	for (; *ctrl; ctrl++) map[*ctrl >> 3] |= (unsigned char)(1 << (*ctrl & 7));
...
//       форматирование = 
//		   s = имя из списка определения типа;
//    2) ItemSetByte - список имен значений битов для перечислимого типа, 
//       начиная с младшего:
//       представление  = значение размера 1 байт;
//       форматирование = 
//         s = комбинация имен из определения списка;
...
//       представление  = значение размера 4 байта;
//       форматирование (при доступности .PDB-файла) = 
//        s = имя из C++-определения перечисления;
//    8) ItemFlagsEnum - перечисление C++ взаимодополняющих флагов:
//       представление  = значение размера 4 байта;
//       форматирование (при доступности .PDB-файла) = 
//         s = комбинация имен из C++-определения перечисления;
...
//          02x = шестнадцатеричное строчное представление;
//          02X = шестнадцатеричное прописное представление; 
...
//              (как у тэгов выделения памяти в драйверах); 
...
// спецификации вида %<NAME> (при указании директивы -noshrieks препроцессору) 
// и %!<NAME>!. Извлеченное имя <NAME> является именем типа WPP, описание 
// которого содержится в стандартном конфигурационном файле defaultwpp.ini. 
// В отличии от типа MOF, который допускает различные способы форматирования, 
// тип WPP связан с единственным способом форматирования (и единственным типом 
// MOF). Типы WPP делятся на 3 категории: перечислимые, простейшие (имеющие 
// фиксированный размер) и составные (имеющие переменный размер). 
// 
// Каждый тип WPP определяется следующими атрибутами: 
// 1) Name       - имя типа WPP; 
// 2) MofType    - имя соответствующего типа MOF;
// 3) FormatSpec - способ форматирования типа MOF; 
// 4) Sig        - суффикс типа WPP при создании оберточных функций (см. ниже); 
// 5) EquivType  - тип C++ формального параметра создаваемой оберточной функции 
//                 (см. ниже); 
// 6) MacroName  - имя макроса, преобразующего параметр оберточной функции в 
//                 пару (размер, адрес) для передачи функции трассировки 
//                 (см. ниже). 
// В конфигурационном файле перечислимые типы определяются через макрос 
// CUSTOM_TYPE, простейшие типы - через макросы DEFINE_SIMPLE_TYPE и 
// DEFINE_SIMPLE_TYPE_PTR, а составные типы - через макрос DEFINE_CPLX_TYPE. 
// 
// При раскрытии макроса трассировки препроцессору WPP приходится генерировать 
// оберточные функции WPP_SF_<Sig1>_..._<SigN>, которые через свои формальные 
// параметры принимают параметры строки форматирования, а тело оберточной 
// функции через преобразующие макросы раскрывает переданные параметры в пары 
// (адрес, размер), передаваемые функции трассировки, которая определяется 
// макросом WPP_TRACE и при отсутствии его переопределения является функцией 
// TraceMessage. 
// 
// Имя оберточной функции WPP_SF_<Sig1>_..._<SigN> формируется на основе 
// суффиксов Sig типов WPP, используемых в строке форматирования. Для 
// простейших и составных типов суффиксы указаны в макросах определения 
// (DEFINE_SIMPLE_TYPE, DEFINE_SIMPLE_TYPE_PTR и DEFINE_CPLX_TYPE). Для 
// перечислимых типов размером 1 байт суффиксом является "c", размером 
// 2 байта - "H", размером 4 байта - "L".  
// 
// Типами формальных параметров оберточной функции являются типы C++, указанные 
// в атрибутах EquivType простейших и составных типов WPP, используемых в 
// строке форматирования. Для простейших и составных типов атрибуты EquivType 
// указаны в макросах определения (DEFINE_SIMPLE_TYPE, DEFINE_SIMPLE_TYPE_PTR и 
// DEFINE_CPLX_TYPE) Для перечислимых типов размером 1 байт типом является 
// signed char, размером 2 байта - unsigned short, размера 4 байта - 
// unsigned long. 
// 
// Имена преобразующих макросов являются значениями атрибутов MacroName типов 
// WPP, используемых в строке форматирования. Для перечислимых и простейших 
// типов преобразующими макросами являются макросы WPP_LOGTYPEVAL и 
// WPP_LOGTYPEPTR. Макрос WPP_LOGTYPEVAL создает пару (адрес, размер) для 
// перечислимых типов и простейших типов, передаваемых по значению (определение 
// которых производится через макрос DEFINE_SIMPLE_TYPE), а макрос 
// WPP_LOGTYPEPTR создает пару (адрес, размер) для простейших типов, 
// передаваемых через указатель (определение которых производится через макрос 
// DEFINE_SIMPLE_TYPE_PTR). Для составных типов преобразующие макросы 
// должны определяться отдельно и их имя (атрибут MacroName) должно быть 
// указано в макросе определения составного типа (DEFINE_CPLX_TYPE). 
// Преобразующие макросы должны определяться на основе макроса WPP_LOGPAIR, 
// который преобразуется в отдельную пару (адрес, размер). 
// 
...
// можно переопределить при помощи директив WPP-препроцессора USEPREFIX и 
...
// 
// На основе созданного .PDB-файла утилитой TracePDB создаются файлы *.TMC и 
// *.TMF. TMC-файлы содержат описание отдельных используемых провайдеров 
// трассировки. TMF-файлы содержат TMF-строки форматирования отдельных единиц 
// компиляции и соответствие всех их параметров типам MOF. 
...
//    Name       = имя типа WPP; 
//    EquivType  = тип C++ формального параметра создаваемой оберточной функции;  
//    MofType    = имя типа MOF, который будет подставляться в TMF-описание; 
//    FormatSpec = способ форматирования типа MOF; 
//    Sig        = суффикс типа WPP при создании оберточных функций;
...
//    Name       = имя типа WPP; 
//    BaseType   = базовый тип WPP, параметры определения которого можно 
//                 переопределить параметрами MofType и/или FormatSpec; 
//    MofType    = имя типа MOF, который будет подставляться в TMF-описание;
//    FormatSpec = способ форматирования типа MOF.
// 
...
//    FormatSpec = способ форматирования типа MOF.
// 
// Определение DEFINE_CPLX_TYPE задает имя преобразующего макроса, который 
...
{ 
	// извлечь идентификатор
	long status = valist_extract<LONG>(args);
...
inline const wpp_format_entry* wpp_find_format(const char* format, const char* separators)
{
	// найти позицию завершения формата
	const char* szEnd = format + strcspn(format, separators); 
